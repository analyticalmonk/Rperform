...
plot_metrics <- function(test_path, metric, num_commits = 5, save_data = FALSE, save_plots = FALSE,
                         interactive = FALSE) {
  .input_check(save_data, save_plots, interactive, num_commits, test_path, metric)
  
  floor(num_commits)
  
  .metrics(metric, interactive, test_path, num_commits, save_data, save_plots)
}
...
plot_webpage <- function(test_directory = "tests/testthat", metric = "testMetrics",
  ...
  .input_check(save_data = FALSE, save_plots = FALSE, interactive = FALSE, 
               num_commits = 0, test_directory, metric, output_name)
...
plot_directory <- function(test_directory, metric = "testMetrics", num_commits = 5, save_data = FALSE,
  ...
  .input_check(save_data, save_plots, interactive = FALSE, num_commits, test_directory, metric)
...
plot_branchmetrics <- function(test_path, metric, branch1, branch2 = "master",
  ...
  .input_check(save_data, save_plots, interactive = FALSE, num_commits = 0, test_path, metric, 
               branch1, branch2)
  
  .metrics_branch(metric, test_path, branch1, branch2, save_data, save_plots)
}
...
.input_check <- function(save_data, save_plots, interactive, num_commits, ...){
  ...
  stopifnot(length(interactive) == 1)
  
  stopifnot(is.numeric(num_commits))
  stopifnot(length(num_commits) == 1)
  
  for(chr_var in list(...)){
    stopifnot(is.character(chr_var))
    stopifnot(length(chr_var) == 1)
  }
}

.metrics <- function(metric, interactive, test_path, num_commits, save_data, save_plots){
  if (metric == "time") {
    if (interactive) {
...
.metrics_branch <- function(metric, test_path, branch1, branch2, save_data, save_plots){
  if (metric == "time") {
