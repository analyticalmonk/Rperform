...
The following examples illustrates the use of the `Rperform::plot_time()` and `Rperform::plot_mem()` functions on the git repository of the package [stringr](https://github.com/tdhock/stringr).

```r
> setwd("./stringr")
> library(Rperform)
> plot_time(test_path = "./tests/testthat/test-extract.r", num_commits = 10, save_data = FALSE)
```
![time plot](images/Rplot_time.jpeg)

```r
> Rperform::plot_mem(test_path = "./tests/testthat/test-dup.r", num_commits = 4, save_data = F)
```
